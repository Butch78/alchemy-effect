...
//
// We use a vendored copy of foreground-child (./_foreground-child.js) so we
// can pipe the child's stderr through a filter — upstream hardcodes
// stdio = [0, 1, 2], so the only way to drop bun's hard-coded watcher
// warning ("warn: File <path> is not in the project directory and will not
// be watched", which is noise in our monorepo and has no bun flag to
// silence) is to own the spawn call. Everything else — signal proxying,
// watchdog, IPC bridging, exit-code forwarding — matches upstream.
import { fileURLToPath } from "node:url";
import path from "pathe";
import { foregroundChild } from "./_foreground-child.js";
